...
<%
	...
	while(enu.hasMoreElements()) {
		...
		int quantity = Integer.parseInt(orderBean.getQuantity()); // 수량 가져오기
		int subTotal = price * quantity; // 소개
...
<form action="cartproc.jsp" method="get">
	...
	<tr>
		...
		<td><%=subTotal %></td>
		<td>
			<input type="text" name="quantity" size="10" value="<%=quantity %>" style="text-align: center;">
		</td>
		<td>
			<input style="background-color: silver;" type="button" value="수정" onclick="cartUpdate(this.form)"> / 
			<input style="background-color: gray;" type="button" value="삭제" onclick="cartDelete(this.form)">
		</td>
		<td>
