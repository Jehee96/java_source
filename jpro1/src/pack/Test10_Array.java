...
//		ar = new int[5];
		...
		ar[1] = 20;
		ar[4] = ar[0] + ar[1] + ar[2];
		System.out.println(ar[4]); // 첨자(index)로 각 기억장소를 호출
//		ar[5] = 50; 				    // 런타임 에러: ArrayIndexOutOfBoundsException
		int a = 4, b = 4;
		System.out.println(ar[4] + " " + ar[a] + " " + ar[b] + " " + ar[1 + 3]); // 첨자 변수
//		   	 				30           a=4=30        b=4=30      ar[1+3]=ar[4]=30
		...
		for (String lee : city) {
			...
			int tot = 0;
			for (int i = 0; i < ar2.length; i++) {
				System.out.println(ar2[i] + " "); // 1 2 3 4 5
				tot += ar2[i]; // tot에 ar2[i]값 누적
			}
			System.out.println("ar2 요소 값 총합 : " + tot);

			System.out.println("\n--- 다차원 배열 ---"); // 열 -> 행 -> 면
//			int au[] = new int[12]; 	// 1차원 배열로 요소 수 12개
			int su[][] = new int[3][4]; // 2차원 배열로 요소 수 12개 (4개의 요소를 3줄로 합함. 4*3)
			su[0][0] = 100;
...
//					su[i][j] = ++num;
			...
			System.out.println("가변 배열 : 행마다 요소(열)의 갯수가 동적");
			int[][] scores = new int[2][];		// 2
			scores[0] = new int[2];				// 2
			scores[1] = new int[3];				// 3
			System.out.println(scores.length + " " + scores[0].length + " " + scores[1].length);	// 2 2 3
			
			System.out.println();
			int[][] jum = {{90, 87}, {88, 99, 100}}; 	// 가변배열 선언 후 값 저장
			for(int i = 0; i < jum.length; i++) { 	 	// int[!][@] // [!]에 90, 87 저장, [@]에 88, 99, 100 저장
