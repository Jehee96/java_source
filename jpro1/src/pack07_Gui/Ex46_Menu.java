...
public class Ex46_Menu extends JFrame implements ActionListener {
	...
	public Ex46_Menu() {
		...
		addLayout();		// 버튼 관련 레이아웃
		addMenuLayout();	// 메뉴 관련 레이아웃
	...
	private void addLayout() {
		...
		add("South", panel);	// Frame은 BorderLayout으로
		add("Center", txtArea);
	...
	@Override
	public void actionPerformed(ActionEvent e) { // 배경색
		...
		if(e.getSource() == btnR) {
			...
			// 내장 다이얼로그 박스 호출
			JOptionPane.showMessageDialog(this, "메세지", "알림", JOptionPane.INFORMATION_MESSAGE); // 사용자에게 보여주고싶은 메세지 표시
			
			// Modal dialogbox는 실행을 멈추고 창을 보여준다. 창을 닫으면 처리 진행. 
			System.out.println("계속");
			
		}else if(e.getSource() == mnuOk) {	  // 메뉴
			int result = JOptionPane.showConfirmDialog(this, "버튼을 고르시오", "골라", JOptionPane.YES_NO_CANCEL_OPTION); // 예 아니오 취소버튼이 있고, RETURN값이 있다.
		...
		}else if(e.getSource() == mnuInput) { // 메뉴 키보드로 값 받기
