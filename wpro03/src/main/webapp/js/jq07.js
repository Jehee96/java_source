...
function goFunc(element) {
	//console.log(element);
	ele = element;
	//console.log(ele.id); // id 속성값 출력
	disp = document.getElementById("disp");
	popup = document.getElementById("popup");
	
	xhr = new XMLHttpRequest();
	xhr.open("post", "jq07_data.jsp", true);
...
function clsFunc(element) {
	//console.log("b");
	for(let i = disp.childNodes.length -1; i >=0; i--) { // 하나씩 지우겠다는 의미
