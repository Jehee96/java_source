...
public class Report extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		for(int i =0; i<jlist.size(); i++) {
			...
			if(no.equals(jumsu.getNo())) { // 번호 중복 체크
				out.println("중복되었습니다");   // 중복체크 시 response.sendRedirect보다 위에 있어야함
				response.sendRedirect("myjumsu.html");//// myjumsu로 돌아가
				return;
			}
		}
		
		jlist.add(new Jumsus(no, name, kor, eng));
		...
		for(int i =0; i<jlist.size(); i++) {
			...
			out.println("<tr><td>" + jumsu.getNo() + "</td>");
			out.println("<td>" + jumsu.getName() + "</td>");
